...
if(kind=='integral'){
  equations=lapply(time_series,extract_integral_system(time_series,removeZeros =removeZeros))
}
else{
  equations=lapply(time_series,extract_log_integral_system(time_series))
}
# Now we have lists of equation systems for each time series. We now want to combine them
stacked_equations = stack_equation(equations)
...
stack_equations = function(equations){
  ...
  stacked_right_sides = do.call(rbind,right_sides_to_stack)
