  ...
  #*************************************************************************************
{
  ...
  if(CA$renormalize){
    ...
    permutation.norm1=permuation.data1
    permutation.norm2=permuation.data2
  }
...
DecodeInputCAArguments <-
  function(CA){
    ...
    if    (CA$make.output.table && CA$memory.optimize)
    {
      warning("In the memory optimized algorithm, generation of table-format output is disabled due to performance reasons
              Try generating the table yourself after the algorithm has returned")
    }
    if (!is.na(CA$concurrent.output) && CA$memory.optimize){
      warning("Concurrent output to file is always disabled in memory optimization mode")
    }
